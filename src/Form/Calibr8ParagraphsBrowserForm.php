...
namespace Drupal\calibr8_paragraphs\Form;
 ...
 */
class Calibr8ParagraphsBrowserForm extends ParagraphsBrowserForm {

  /**
   * {@inheritdoc}
   */
  public function getFormId() {
    return 'calibr8_paragraphs_browser_form';
  }
    ...
    if ($paragraph = $build_info['paragraph']) {
      ...
      foreach ($paragraph_items as $delta => $paragraph_item) {
        ...
        if ($paragraph_item['target_id'] == $triggering_paragraph->id()) {
          if ($default_paragraph_id = \Drupal::state()->get('calibr8_paragraphs_' . $type . '_default')) {
      ...
      $identifier = '[data-calibr8-paragraphs=' . $parent_field_name . '-' . $parent->id() . ']';
      $response = new AjaxResponse();
